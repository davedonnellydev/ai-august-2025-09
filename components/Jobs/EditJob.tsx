...
export function EditJob({ jobId, userId, onBack, onSave }: EditJobProps) {
    ...
    try {
      ...
      if (result.success) {
        ...
        setFormData({
          ...
          salaryMin: job.salaryMin ? parseInt(job.salaryMin, 10) : '',
          salaryMax: job.salaryMax ? parseInt(job.salaryMax, 10) : '',
          ...
          decision: job.decision || 'undecided',
        });
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        setSuccess(true);
        // Redirect back to job view after a short delay
        setTimeout(() => {
          onSave();
        }, 2000);
      } else {
    ...
    return (
      <Container size="xl" py="xl">
        <Card p="xl" ta="center">
          ...
          <Text c="dimmed">Redirecting back to job details...</Text>
        </Card>
      </Container>
    );
  }
            ...
            <Stack gap="md">
              <Group grow>
                <Select
                  ...
                  onChange={(value) =>
                    handleInputChange('status', value || 'new')
                  }
                />
                ...
                <Select
                  ...
                  onChange={(value) =>
                    handleInputChange('decision', value || 'undecided')
                  }
                />
              </Group>
            </Stack>
