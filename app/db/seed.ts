...
async function main() {
  ...
  console.log(
    '✅ Cleared cron-synced tables (email_links, email_messages, gmail_sync_state)'
  );
  ...
  const [label] = await db
    ...
    .onConflictDoUpdate({
      target: [
        gmailLabelsCacheTable.userId,
        gmailLabelsCacheTable.providerLabelId,
      ],
      set: {
        name: 'Job Leads',
        type: 'user',
    ...
    .returning();
  console.log('✅ gmail_labels_cache:', {
    id: label.id,
