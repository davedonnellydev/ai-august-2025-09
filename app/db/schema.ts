...
export const gmailLabelsCacheTable = pgTable(
  'gmail_labels_cache',
  {
...
export const userSettingsTable = pgTable('user_settings', {
  userId: uuid('user_id')
    .primaryKey()
    .references(() => usersTable.id, { onDelete: 'cascade' }),
  watchedLabelIds: jsonb('watched_label_ids')
    .$type<string[]>()
    .notNull()
    .default([]), // Gmail label IDs
  cronFrequencyMinutes: integer('cron_frequency_minutes')
    .notNull()
    .default(1440), // 60..1440
...
export const syncStateTable = pgTable(
  ...
  (t) => [index('idx_runs_user_time').on(t.userId, t.startedAt)]
);
